<template>
    <div v-if="roomInfo">
        <div class="chat-header-wrap flex items-center gap-3 px-5 py-base-2 rounded-t-none md:rounded-t-base-lg">
            <router-link :to="{ name: this.$route.name, force: true}" v-if="isMobile" class="text-inherit rtl:rotate-180">
            ...
            <button v-if="roomInfo.enable && roomInfo.status === 'accepted'" @click="openDropdownMenu()">
                <BaseIcon name="more_horiz_outlined" size="20" class="chat-header-wrap-icon text-sub-color dark:text-slate-400"></BaseIcon>                
            </button>
        </div>
    </div>
</template>
