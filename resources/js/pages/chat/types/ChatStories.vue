<template>
    <div :class="{'text-end ms-auto': owner}">
        <div v-if="owner" class="inline-block bg-web-wash text-xs px-2 py-1 mb-1 rounded-md dark:bg-dark-web-wash">{{$t("You replied to their story")}}</div>
        <div v-else class="inline-block bg-web-wash text-xs px-2 py-1 mb-1 rounded-md dark:bg-dark-web-wash">{{ message.user.name + ' ' + $t('replied to your story')}}</div>
        <div v-if="messageItems" class="flex flex-wrap gap-2" :class="{'justify-end': owner}">
