<template>
    <div class="opacity-0 transition-opacity flex gap-1 group-hover:opacity-100 relative">
        <button v-if="!message.is_delete && owner" class="flex items-center justify-center w-7 h-7 rounded-full transition-colors hover:bg-gray-6 dark:hover:bg-slate-600 cursor-pointer" v-tooltip.top="{value: $t('Unsent'), showDelay: 1000}" @click="unsentMessage(message)">
