    ...
    <div>
        ...
        <div>
            ...
            <div v-if="loadMoreStatus" class="text-center mt-5">
                <BaseButton @click="getPagesList(searchPage, searchKeyword, formatedSelectedCategory)">{{$t('View more')}}</BaseButton>
            </div>
        </div>
    </div>
</template>
...
export default {
    props: ['categoryId'],
    components: { Loading, UserPagesList, BaseButton, BaseInputText, TreeSelect },
    data(){
        return {
            searchPage: 1,
            searchKeyword: '',
