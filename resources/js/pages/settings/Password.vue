<template>
    <form @submit.prevent="changePassword">
		<div class="flex flex-wrap gap-x-5 mb-3"> 
			<div class="md:flex-1 md:text-end w-full mb-1 pt-2"><label>{{$t('Old Password')}}</label></div>
			<div class="md:flex-2 w-full">
                <BasePassword v-model="passwordData.password" :error="error.password" :left_icon="false" />			
			</div>  
		</div>
		<div class="flex flex-wrap gap-x-5 mb-3"> 
			<div class="md:flex-1 md:text-end w-full mb-1 pt-2"><label>{{$t('New Password')}}</label></div>
			<div class="md:flex-2 w-full">
