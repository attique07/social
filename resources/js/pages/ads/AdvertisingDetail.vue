<template>
    <FullColumn :keep-right-column="true">
            ...
            <div class="main-content-section bg-white border border-white text-main-color rounded-none md:rounded-base-lg p-5 dark:bg-slate-800 dark:border-slate-800 dark:text-white">
                ...
                <div v-else class="flex gap-base-2 justify-between mb-base-2">
                    <div class="flex-1">
                        ...
                        <div class="text-base-xs text-sub-color dark:text-slate-400">{{ adsInfo.created_at }}</div>
                    </div>
        ...
        <template v-slot:right>
            <div v-if="adsInfo" class="main-content-section bg-white border border-white text-main-color rounded-none md:rounded-base-lg p-5 dark:bg-slate-800 dark:border-slate-800 dark:text-white">
                ...
                <div>
                    <div class="feed-item-header flex mb-base-2">
                        ...
                        <div class="feed-item-header-info flex-grow ps-base-2">
                            <div class="feed-item-header-info-title flex justify-between items-start gap-base-2">
                                ...
                                <router-link :to="{name: 'post', params: {id: adsInfo.post.id}}" class="pt-[0.1rem]">
                                    <BaseIcon name="more_horiz_outlined" size="20" class="feed-item-dropdown text-sub-color dark:text-slate-400"/>
                                </router-link>
                            </div>
                            <router-link :to="{name: 'post', params: {id: adsInfo.post.id}}" class="feed-item-header-info-date block w-max text-sub-color text-xs dark:text-slate-400">{{ adsInfo.created_at }}</router-link>
                        </div>
                    </div>
                    <div class="feed-item-content">
                        <ContentHtml :content="adsContent" />       
                    ...
                    <div class="feed-main-action mt-3 mb-1 dark:text-white">
                        ...
                        <router-link :to="{name: 'post', params: {id: adsInfo.post.id}}" class="feed-item-like-text inline-block text-main-color font-semibold dark:text-white">{{ $filters.numberShortener(adsInfo.post.like_count, $t('[number] like'), $t('[number] likes')) }}</router-link>
                    </div>
                    <div v-if="adsInfo.post.comment_count > 0" class="feed-comment-info-comment-holder-view-all text-sub-color dark:text-slate-400 ">              
                        <router-link :to="{name: 'post', params: {id: adsInfo.post.id}}" class="text-inherit">
                            {{ $filters.numberShortener(adsInfo.post.comment_count, $t('View all [number] comment'), $t('View all [number] comments')) }}
                        </router-link>              
                    </div>
