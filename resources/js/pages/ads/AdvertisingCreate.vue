        ...
        <template v-slot:center>
            <div v-if="adsInfoRendered" class="main-content-section bg-white border border-white text-main-color rounded-none md:rounded-base-lg p-5 mb-base-2 dark:bg-slate-800 dark:border-slate-800 dark:text-white">
                ...
                <div class="mb-base-2">
                    ...
                    <Mentionable v-model="adsContent" @update:modelValue="inputChange" :rows="3" ref="mention" :hasEventEnter="false" className="p-inputtext pb-8" :error="error.content">
                        <EmojiPicker @emoji_click="addEmoji" class="absolute left-2 bottom-2"/>
                    </Mentionable>
                </div>
                <ProgressBar :value="videoUploadProgress" class="mb-base-2"/>
                <div v-if="showUploadImage || showUploadVideo || showUploadFile">
                    <label class="block mb-1">{{ $filters.textTranslate(config.videoDurationConvertNow, $t('Select your media (Can upload ONE %s seconds video)')) }}</label>
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-base-2">
                        <div v-if="showUploadImage" class="relative">
                            <label for="imagesUploadStatus" class="flex items-center gap-base-2 font-medium">
                                <BaseIcon name="camera" />
                                {{ $t('Photos') }}
                            </label>
                            <input type="file" title="" @change="uploadImages($event)" class="absolute top-0 left-0 w-full h-full text-base-none opacity-0 cursor-pointer" name="imagesUploadStatus" ref="imagesUploadStatus" multiple accept="image/*" />
                        </div>
                        <div v-if="showUploadVideo" class="status-box-action-list-item relative">
                            <label for="videoUploadStatus" class="flex items-center gap-base-2 font-medium">
                                <BaseIcon name="video_camera" />
                                {{ $t('Video') }}
                            </label>
                            <input type="file" title="" @change="uploadVideo($event)" class="absolute top-0 left-0 w-full h-full text-base-none opacity-0 cursor-pointer" name="videoUploadStatus" ref="videoUploadStatus"  accept="video/*"/>
                        </div>
                        <div v-if="showUploadFile" class="status-box-action-list-item relative">
                            <label for="fileUploadStatus" class="flex items-center gap-base-2 font-medium">
                                <BaseIcon name="paperclip" />
                                {{ $t('Attachment') }}
                            </label>
                            <input type="file" title="" @change="uploadFiles($event)" class="absolute top-0 left-0 w-full h-full text-base-none opacity-0 cursor-pointer" name="fileUploadStatus" ref="fileUploadStatus" multiple accept="*/*"/>
                        </div>
                    </div>
                </div>
                <div v-if="adsMedias.length" class="flex flex-wrap gap-2 mt-base-2 rounded-md border" :class="error.items ? 'border-invalid-color p-2' : 'border-transparent'">
                    ...
                    <div class="relative cursor-pointer group">
                        <label for="fileUploadStatus">
                            <div class="add-images-icon inline-flex items-center justify-center w-20 h-20 bg-cover bg-center text-main-color border border-divider dark:text-white/50 dark:border-white/10 rounded-md group-hover:bg-hover">
                                <span class="leading-none">
                                    <BaseIcon name="photo" />
                                </span>
                            </div>
                        </label>
                        <input type="file" title="" @change="uploadImages($event)" @drop.prevent="dropImage($event)" class="absolute top-0 left-0 w-full h-full text-base-none opacity-0 cursor-pointer" name="fileUploadStatus" ref="fileUploadStatus" multiple accept="image/*" />
                    </div>
                </div>
                ...
                <div v-if="adsConfig" class="mb-base-2">
                    ...
                    <div class="flex flex-wrap gap-x-5 gap-y-base-2">
                        ...
                        <div v-for="(gender, key) in adsConfig.genders" :key="key" class="flex items-center gap-base-2">
                            <BaseRadio :value="parseInt(key)" v-model="genderId" :inputId="gender" name="genders" />
        ...
        <template v-slot:right>
            <div v-if="adsInfoRendered" class="main-content-section bg-white border border-white text-main-color rounded-none md:rounded-base-lg p-5 dark:bg-slate-800 dark:border-slate-800 dark:text-white">
                ...
                <div>
                    ...
                    <div class="feed-main-action mt-3 mb-2 dark:text-white">
...
export default {
    props: ['id'],
    components: { Draggable: VueDraggableNext, FullColumn, Mentionable, AutoComplete, BaseInputText, BaseCalendar, BaseSelectLocation, BaseIcon, Avatar, UserName, InfoTooltip, BaseButton, BaseRadio, LinkFetched, Loading, VueperSlides, VueperSlide, EmojiPicker, ProgressBar },
    data(){
        return{
			...
			for( var i = 0; i < uploadedFiles.length; i++ ){
				...
				if(checkUpload){
					...
					try {
						...
						this.showError(error.error)
                        this.$refs.imagesUploadStatus.value = null
					}	
				}
			}
            this.$refs.imagesUploadStatus.value = null	
			...
			for( var i = 0; i < uploadedFiles.length; i++ ){
				if(this.checkUploadedData(uploadedFiles[i], 'video')){
					...
					try {
						...
						this.$refs.videoUploadStatus.value = null
			...
			for( var i = 0; i < uploadedFiles.length; i++ ){
				if(this.checkUploadedData(uploadedFiles[i], 'post')){
					...
					try {
						...
						this.showError(error.error)
						this.adsLoadingUploadFiles.shift()
                        this.$refs.fileUploadStatus.value = null
					}	
				}
			}
            this.$refs.fileUploadStatus.value = null
