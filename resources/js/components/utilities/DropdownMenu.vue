<template>
	<div v-if="appendTo === 'body'" class="inline-block">
        ...
        <OverlayPanel class="dropdown-menu-box bg-white text-main-color shadow-sidebar-more rounded-base dark:text-white dark:bg-slate-800" :style="{'marginStart': offsetX+'px', 'marginTop': offsetY+'px'}" ref="dropdown_menu" @click.stop="handleClickOverlayPanel">        
            <slot></slot>
        </OverlayPanel>
    </div>
	<div v-else>
...
export default {
	...
	methods: {
		...
		updateCaretPosition () {
			...
			if(dropdownButtonRect.left > this.measuredWidth){
			...
			if((window.innerHeight - dropdownButtonRect.top) > this.measuredHeight){
				this.caretPosition.top = dropdownButtonRect.height + this.offsetY
				this.caretPosition.bottom = null
			}else{
				this.caretPosition.bottom = dropdownButtonRect.height + this.offsetY
