        ...
        <Teleport to="body">
            <Transition name="fade">
                ...
                :style="caretPosition ?
                {
                    ...
                    <div class="header-profile bg-white dark:bg-slate-800 dark:text-white shadow-lg rounded-none md:rounded-base ">
                        ...
                        <div class="relative inline-block ms-base-2 -mt-14 p-1 rounded-full" :class="(!userData.story_id || !authenticated) ? 'bg-white dark:bg-slate-800' : 'bg-gradient-story cursor-pointer'">
                            <router-link :to="{name: 'profile', params: {user_name: userData.user_name}}">
                                <img class="w-20 h-20 rounded-full" :src="userData.avatar" />
                            </router-link>
                        </div>
                        <div class="px-base-2 pb-base-2">
                            <div class="flex justify-between items-start gap-5">
                                <div class="w-full">
                                    <router-link :to="{name: 'profile', params: {user_name: userData.user_name}}" class="header-profile-name text-inherit text-base-lg font-medium truncate block w-full">
                                         <div class="flex items-center gap-1">
                                            <span class="truncate">{{userData.name}} </span>
                                            <template v-if="config.userVerifyEnable && userData.is_verify">
                                                <img v-if="!userData.is_page" class="w-4 h-4" :src="config.userVerifyBadgeIcon"/>
                                                <img v-if="userData.is_page" class="w-4 h-4" :src="config.userPageVerifyBadgeIcon"/>
                                            </template>
                                            <img v-if="userData.is_page && userData.is_page_feature" class="w-4 h-4" :src="config.user_page.featureBadgeIcon"/>
                                        </div>
                                    </router-link>
                                    ...
                                    <div v-if="userData.check_privacy" class="header-profile-count text-base-sm mb-base-1">
                                        <span v-if="userData.show_following" class="inline-block">{{ $filters.numberShortener(userData.following_count, $t('[number] Following'), $t('[number] Followings')) }}</span> <span v-if="userData.show_following && userData.show_follower">.</span> <span v-if="userData.show_follower" class="inline-block">{{ $filters.numberShortener(userData.follower_count, $t('[number] Follower'), $t('[number] Followers')) }}</span>
                                    </div>
                                </div>
                            </div>
...
export default {
    components: { BaseButton, MessageButton },
    data(){
        return{
