        ...
        <div class="h-full w-full relative">
            <div class="absolute top-4 left-2 right-2 lg:left-4 lg:right-4 flex gap-1 z-10">
                <div v-for="item in story.items" :key="item.id" class="flex-1 bg-gray-400 rounded-full story-remaining-bar">
                    ...
                    <div v-if="storyItem.id == item.id" class="rounded-full h-1 w-full bg-progress story-progress-bar" :style="{'width': (time * 100 / config.storyTimeout) +'%'}"></div>
                </div>
            </div>
            <StoryItemDetail :canMessage="story.canMessage" :story="storyItem" :owner="story.user" :runningStory="run" :isPage="isPage" @play_story="doPlayStory" @stop_story="stopPlayStory" @close_story_modal="$emit('closeModal')"/>
        </div>
...
export default {
    ...
    methods: {        
        ...
        doPlayStory(){
            if (parseInt(this.time) >= this.config.storyTimeout ) {
                this.time = 0                
