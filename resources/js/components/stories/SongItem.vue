	...
	<div class="flex gap-4 items-center justify-between mb-2 last:mb-0">
		<span class="flex gap-base-2 items-center justify-between w-full">
			<button class="flex gap-base-2 items-center">
				<BaseIcon name="pause_song" size="20" :id="'pause'+song.id" class="pauseBtn bg-primary-color p-base-1 text-white rounded-full dark:bg-dark-primary-color" :style="{ display: 'none' }" @click="pauseSong(song.id)" />
				<BaseIcon name="play_song" size="20" :id="'play'+song.id" class="playBtn bg-primary-color p-base-1 text-white rounded-full dark:bg-dark-primary-color" @click="playSong(song.id)" />
			</button>
			<div class="flex-1 truncate">{{ song.name }}</div>
			<BaseButton @click="selectSongItem(song)" size="sm">{{ $t('Select') }}</BaseButton>
		</span>
