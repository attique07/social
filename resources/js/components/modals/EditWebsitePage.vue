<template>
    <div>
        <div v-for="(website, index) in websites" :key="index" class="mb-3 relative">
            <BaseInputText v-model="websites[index]" :class="{'pe-8': websites.length > 1}" :autofocus="true"/>
            <button v-if="websites.length > 1" @click="removeMoreLink(index)" class="absolute top-2 end-1"><BaseIcon name="close" class="text-red-600"/></button>
        </div>
