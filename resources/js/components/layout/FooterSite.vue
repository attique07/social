<template>
	<div class="main-footer-menu py-4 text-sub-color dark:text-gray-300 dark:border-white/10">
		...
		<span class="mt-3">{{$t('Copyright 2022 by shaunSocial')}}</span>
		.
		<button @click="showMenuLanguage()" class="text-xs font-bold text-primary-color dark:text-dark-primary-color">{{ getActiveLanguage }}</button>
	</div>
</template>
<script>
