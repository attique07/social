<template>
    <div v-if="pagesListData.length" :class="`user_pages_list_${positionWidget}`">
        <div v-if="listStyle === 'list'">
            <div v-for="page in pagesListData" :key="page.id" class="users-list-item flex items-center gap-base-2 mb-base-2">
        ...
        <VueperSlides v-else class="no-shadow relative" :slide-ratio="slideRatio" :visible-slides="isMobile ? 1 : 2" slide-multiple :gap="1" :autoplay="true" :duration="6000" :bullets="false" :touchable="false" :rtl="user.rtl ? true : false">
                        ...
                        <div class="rounded-lg border border-divider dark:border-white/10 h-full">
                            ...
                            <div class="p-base-2">
                                <div class="flex gap-4">
                                    <Avatar :user="page" :activePopover="false" :size="'3.25rem'"/>
                                    <div class="flex-1 min-w-0">
            ...
            <template #arrow-left>
                <div class="flex items-center justify-center w-10 h-10 bg-white shadow-md rounded-full text-main-color dark:bg-slate-800 dark:text-white dark:shadow-slate-500 z-10"><BaseIcon name="caret_left"/></div>
            </template>
            <template #arrow-right>
                <div class="flex items-center justify-center w-10 h-10 bg-white shadow-md rounded-full text-main-color dark:bg-slate-800 dark:text-white dark:shadow-slate-500 z-10"><BaseIcon name="caret_right"/></div>
            </template>
        </VueperSlides>
    </div>
            ...
            if(this.listStyle !== 'list'){
                this.$nextTick(() => {
                    const ratios = this.pagesListData.map((page, index) => {
                        ...
                        return  contentHeight / (content[0].offsetWidth * ( this.isMobile ? 1 : 2));
                    });
