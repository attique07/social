...
('Allow create page', 'user_page.allow_create', 0, (SELECT `id` FROM `{prefix}permission_groups` WHERE `key` = 'user_page'), NOW(), NOW(), 'checkbox', 0, (SELECT MAX(p.`order`) + 1 FROM `{prefix}permissions` as p), '');
...
('page_feature', 1, 'Packages\\ShaunSocial\\UserPage\\Subscription\\SubscriptionTypePageFeature');
...
('user_page_feature_buy', 'Packages\\ShaunSocial\\UserPage\\Wallet\\Payment\\UserPageTransactionTypeBuyFeature');
