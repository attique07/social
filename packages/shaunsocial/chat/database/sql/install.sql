...
('Allow message', 'chat.allow', 0, (SELECT `id` FROM `{prefix}permission_groups` WHERE `key` = 'chat'), NOW(), NOW(), 'checkbox', 0, (SELECT MAX(p.`order`) + 1 FROM `{prefix}permissions` as p), '');
