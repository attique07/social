...
('permissions', 'message_error', (SELECT `id` FROM `{prefix}permissions` WHERE `key` = 'post.character_max') ,'en', 'You only can share a post that has max [x] characters.'),
('permissions', 'message_error', (SELECT `id` FROM `{prefix}permissions` WHERE `key` = 'post.video_max_duration') ,'en', 'You only can share a video that has max [x] seconds.'),
