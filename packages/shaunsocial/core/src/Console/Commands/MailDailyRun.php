...
class MailDailyRun extends Command
{
    ...
    {
        $dailyEmails = UserDailyEmail::where('created_at', '<', now()->subDays(setting('site.mail_daily.day')))->limit(config('shaun_core.mail_daily.limit_run'));
        $dailyEmails->each(function($dailyEmail){
            ...
            if ($user && ! $user->getMailUnsubscribe()) {
                ...
                $results = $builder->get();
                if (count($results)) {
                    $notifications = $this->filterNotification($results);
