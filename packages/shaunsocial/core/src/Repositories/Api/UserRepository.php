...
class UserRepository
{
    ...
    {       
        ...
        $users = Cache::remember($key, config('shaun_core.cache.time.short'), function () use ($query, $data, $viewer) {
            $builder = User::where('user_name', 'LIKE', '%'.$query.'%')->where('is_active', true)->orderBy(DB::raw("LOCATE('".$query."', user_name)"))->limit(setting('feature.item_per_page'));
            if (!empty($data['only_user'])) {
    ...
    public function filterUserSuggestList($users, $viewer = null)
    {
        return $users->filter(function ($value, $key) use ($viewer) {
            return $viewer ? !$viewer->getFollow($value->id): true;
        })->values();
    }
    ...
    {
        return array_merge(['enable_notify' => $viewer->enable_notify ? true : false], ['post' => $viewer->getPostNotifySetting(), 'system' => $viewer->getSystemNotifySetting(),'follow' => $viewer->getFollowNotifySetting(), 'chat' => $viewer->getChatNotifySetting(), 'story' => $viewer->getStoryNotifySetting()]);
    }
