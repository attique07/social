...
<section class="modal-card-body">        
    <div class="card-content">
        <form id="subscription_plan_form" method="post" enctype="multipart/form-data" action="{{ route('admin.user_subscription.plan.store')}}">
            <div id="errors"></div>
            ...
            <div class="form-group">
                ...
                <select name="billing_cycle_type" class="form-select">
                    @foreach ($billingCycleType as $value => $billingCycleName)
                        <option @if ($value == $plan->billing_cycle_type) selected @endif value="{{$value}}">{{$billingCycleName}}</option>
                    @endforeach
                </select>
                <p class="help">
